...
/obj/item/proc/default_sword_parry(mob/living/user, var/damage, atom/damage_source = null, mob/attacker = null, var/def_zone = null, var/attack_text = "the attack")
	...
	if(default_parry_check(user, attacker, damage_source) && prob((block_chance + (user.skills["melee"]))) && (user.get_active_hand() == src))//You gotta be holding onto that sheesh bro.
