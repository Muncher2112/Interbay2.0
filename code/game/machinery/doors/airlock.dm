...
/obj/machinery/door/airlock/attackby(var/obj/item/C, var/mob/user)
	...
	if(!repairing && isWelder(C) && !( src.operating > 0 ) && src.density)
		...
		else if(user.statscheck(user.str, 10, 1, message = "Gah, I'm not strong enough to open the door. Maybe if I try again."))//Gotta be strong to get that there door open.
			if(density)
				spawn(0)	open(1)
			else
				spawn(0)	close(1)
